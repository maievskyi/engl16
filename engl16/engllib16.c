...
(newamountword) * sizeof(struct word), (newamountword));			//
 ...
 /*
 /////////////////// ========  запись в файл(из ком стр) базу несорт-х структ с нов имен =======/////////////
 char* writebase(const char **argv, struct word *pmemword, int countnumword)	//argv[0] -для отладки
 // pmemword - указ на массив несорт структур, countnumword - число несорт структур
 //, возврат указ имя файла с несорт структур ( pnamenosort )
 {
 printf("   ~~~ Запись в файл несортированного массива структур по адресу: ~~~  \n");
 ...
 char *pnamewordnosort;  // указат на дин строка-имя читаемого файла "argv[1]_nosort.dat"
 pnamewordnosort = rename2(argv[1], "_nosort.dat",4);
 //.....................................................................................................

 //~~~~~~~~~~~~~~ создание нов файл в котором сохранять базу слов- "argv[1]_nosort.dat"  ~~~~~~~~~~~~~~~
 errno_t err = 1;   // зачем 1 -  ?????????????????????????????????? отлака наверно
 FILE *nosortfile;		// нов файл в котором сохранять базу слов- "argv[1]_nosort.dat"
 err = fopen_s(&nosortfile, pnamewordnosort, "w+b");	// созд нов бинарн файл для чт и записи
 //      в котором сохранять базу слов-  "argv[1]_nosort.dat"
					...
					//ф-я сама открывает r+и и потом закрывает файл на hdd 
{
 ...
 ///////////////////////===========  Алфавитная сортировка 2  =============////////////////////////////
 struct word * alphabet2(struct word *aa, int *bb)// возврат указатель на память с отсортир по алфав массивом структ,
 // a дин память с маc структур,   b количество структур
 {
 int(*pmeasure)(const void *, const void *);  //обьявл указатель на функцию для передачи параметром в  ф-ю qsort()
 pmeasure = measurealph;						//  и присвоение указателю адреса функции
...
struct word * reduct3(struct word *pa, int *psize)	//сокращает повторяющиеся слова после алф сорт
